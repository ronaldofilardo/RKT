...
export function TimelineScreen({ data }: { data: any }) {
...
export function FinishedBanner({ data }: { data: any }) {
  const { match, matchId, router, winner, scoreState, abandonCurrentSession, p1SetsWon, p2SetsWon } = data;
  if (!data.isFinished) return null;
  const winnerName = winner === 'player1' ? match.player1.name : match.player2.name;
  return <div className="mt-2 sm:mt-3 bg-yellow-500/20 border-2 border-yellow-400 rounded-2xl p-3 sm:p-5 text-center relative z-10 mx-0"><span className="text-2xl sm:text-4xl">🏆</span><h2 className="text-base sm:text-xl font-black text-white mt-1 sm:mt-2">PARTIDA FINALIZADA!</h2><p className="text-sm sm:text-lg font-bold text-yellow-300 mt-0.5 sm:mt-1">VENCEDOR: {winnerName}</p><p className="text-[11px] sm:text-sm text-gray-400 mt-0.5 sm:mt-1">{scoreState?.setsWon.player1 ?? p1SetsWon} x {scoreState?.setsWon.player2 ?? p2SetsWon} sets</p><div className="flex gap-2 sm:gap-3 mt-2 sm:mt-4 justify-center"><button onClick={() => router.push(`/match/${matchId}/report`)} className="flex-1 sm:flex-none px-3 sm:px-5 py-2.5 sm:py-2 bg-yellow-500 hover:bg-yellow-400 text-gray-900 font-bold rounded-xl text-[11px] sm:text-sm min-h-[44px]">📊 Relatório</button><button onClick={async () => { await abandonCurrentSession(); router.push('/dashboard'); }} className="flex-1 sm:flex-none px-3 sm:px-5 py-2.5 sm:py-2 bg-white/10 hover:bg-white/20 text-white font-bold rounded-xl text-[11px] sm:text-sm border border-white/20 min-h-[44px]">✅ Registrar</button></div></div>;
}

export function ScoringScreen({ data }: { data: any }) {
...
function SetupModalView({ data }: { data: any }) {
...
function ServeEffectModalView({ data }: { data: any }) {
...
function PointDetailsModalView({ data }: { data: any }) {
...
function ModalStack({ data }: { data: any }) {
  const { state, handlers, derived, match } = data;

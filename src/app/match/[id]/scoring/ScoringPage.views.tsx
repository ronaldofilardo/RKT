...
export function ScoringScreen({ data }: { data: ScoringViewData }) {
  ...
  const { effectiveScoreState, p1IsServing, p2IsServing, isMatchPoint, isSetPoint, isBreakPoint, isTiebreak, isSuperTiebreak, isFinished, winner, canUndo, isProcessingPoint } = derived;
  const { elapsed, fontScale, pointsHistory, suspendedSession, sessionIdRef, sessionActive, setViewMode, setFontScale } = state;
  const { abandonCurrentSession, openAceModal, handleAceDirect, handleServeErrorWithModal, handleServeErrorDirect, handleVoltar, handlePointFromCard, handleEditScore, handleEditScoreCancel, handleEditScoreRefreshFloor, handleSetupConfirm, handleUndo } = handlers;
  return <div className="min-h-screen bg-gray-900 flex flex-col" style={{ fontSize: `${fontScale * 100}%` }}>{state.syncStatus !== 'synced' && <SyncBanner syncStatus={state.syncStatus} />}<MatchHeader elapsedSeconds={elapsed} startedAt={match?.startedAt} onClose={async () => { await abandonCurrentSession(); router.push('/dashboard'); }} onTimeline={() => setViewMode('timeline')} isFinished={isFinished} format={match.format as string} /><div className="flex-1 flex flex-col gap-0 sm:gap-1 px-2 sm:px-3 py-1 sm:py-2 relative overflow-hidden"><div className="absolute inset-0 opacity-20 pointer-events-none"><CourtBackground courtType={match.courtType} /></div><div className="my-2 sm:my-3"><ScoreboardCard player1={match.player1} player2={match.player2} scoreState={effectiveScoreState} isSuspended={!!suspendedSession} format={match.format as string} /></div><div className="flex items-center gap-1 sm:gap-2 flex-1 relative z-10 min-h-0"><PlayerCard player={match.player1} side="player1" scoreState={effectiveScoreState} isServing={p1IsServing} isSetPoint={isSetPoint} isBreakPoint={isBreakPoint} isWinner={winner === 'player1'} onPoint={() => handlePointFromCard('player1')} onSwipeDown={() => state.open('undo')} disabled={isFinished} /><div className="w-px h-full bg-white/10 flex-shrink-0" /><PlayerCard player={match.player2} side="player2" scoreState={effectiveScoreState} isServing={p2IsServing} isSetPoint={isSetPoint} isBreakPoint={isBreakPoint} isWinner={winner === 'player2'} onPoint={() => handlePointFromCard('player2')} onSwipeDown={() => state.open('undo')} disabled={isFinished} /></div><ContextBadges isMatchPoint={isMatchPoint} isSetPoint={isSetPoint} isBreakPoint={isBreakPoint} isTiebreak={isTiebreak} isSuperTiebreak={isSuperTiebreak} pointsHistory={pointsHistory} /><FinishedBanner data={{ ...data, ...derived }} /></div><ActionBar secondServe={false} serveStep={state.serveErrorState.serveStep} canUndo={canUndo} canEdit={!isFinished} fontScale={fontScale} isFinished={isFinished} isProcessing={isProcessingPoint} onAceDirect={handleAceDirect} onAceWithDetails={openAceModal} onOut={(step) => handleServeErrorWithModal('out', step)} onNet={(step) => handleServeErrorWithModal('net', step)} onOutDirect={(step) => handleServeErrorDirect('out', step)} onNetDirect={(step) => handleServeErrorDirect('net', step)} onVoltar={handleVoltar} onFontSmaller={() => setFontScale((f: number) => Math.max(0.6, f - 0.1))} onFontBigger={() => setFontScale((f: number) => Math.min(2, f + 0.1))} onEditScore={() => state.open('edit-score')} />{sessionIdRef.current && <AnnotationSessionPanel sessionId={sessionIdRef.current} matchId={matchId} isActive={sessionActive} onStart={() => state.setSessionActive(true)} onPause={() => state.setSessionActive(false)} onEnd={async () => { await abandonCurrentSession(); state.setSessionActive(false); }} annotatorCount={1} />}<ModalStack data={{ ...data, ...derived, handleSetupConfirm, handleUndo, handleEditScore, handleEditScoreCancel, handleEditScoreRefreshFloor }} /></div>;
}

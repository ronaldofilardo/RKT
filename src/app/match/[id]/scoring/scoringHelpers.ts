...
export function isSetCompleted(
  ...
  // Match Tie-Break: verificar se o tiebreak foi completado
  const isMatchTiebreakFormat = format === 'BEST_OF_3_MATCH_TB' || format === 'SHORT_SET_2V2_NO_AD' || format === 'MATCH_TB_10';
  
  if (isMatchTiebreakFormat && set.tiebreakScore) {
    const tb = set.tiebreakScore;
    // SHORT_SET_2V2_NO_AD uses regular tiebreak (first to 7), others use match tiebreak (first to 10)
    const tbMin = format === 'SHORT_SET_2V2_NO_AD' ? 7 : 10;
    const p1Won = tb.player1 >= tbMin && tb.player1 - tb.player2 >= 2;
    const p2Won = tb.player2 >= tbMin && tb.player2 - tb.player1 >= 2;
    return p1Won || p2Won;
  }

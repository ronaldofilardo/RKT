...
describe('DashboardPage - Menu Hamburguer', () => {
  ...
  it('deve mostrar contador no item Ao Vivo quando houver partidas ao vivo', async () => {
    setupSessionStorage('user-123', 'ATHLETE');

    // Mock de fetch para retornar partidas ao vivo
    global.fetch = jest.fn(() =>
      Promise.resolve({
        ok: true,
        json: () =>
    ...
    // Aguarda carregar
    await waitFor(() => {
      expect(screen.getByText(/minhas partidas/i)).toBeInTheDocument();
    }, { timeout: 3000 });
    ...
    // Verifica se o contador está presente
    expect(screen.getByText('1')).toBeInTheDocument();
  });

  it('deve mostrar contador no item Aguardando quando houver partidas pendentes', async () => {
    setupSessionStorage('user-123', 'ATHLETE');

    // Mock de fetch para retornar partidas pendentes
    global.fetch = jest.fn(() =>
      Promise.resolve({
        ok: true,
        json: () =>

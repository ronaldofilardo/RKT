...
function BirthDateFields({ form, saving, onFieldChange }: Pick<EditAthleteFieldsProps, 'form' | 'saving' | 'onFieldChange'>) {
  ...
  return <div className="col-span-3"><label htmlFor="edit-athlete-birthday" className="block text-xs font-medium text-gray-700 mb-0.5">Data de Nascimento</label><div className="flex items-center gap-1">{fields.map(([name, id, placeholder, min, max], index) => <span key={name} className="flex items-center gap-1">{index > 0 && <span className="text-gray-500 font-medium text-xs">/</span>}<input id={id} aria-label={placeholder} type="number" min={min} max={max} value={form[name]} onChange={(e) => onFieldChange(name, e.target.value)} disabled={saving} placeholder={placeholder} maxLength={placeholder === 'AAAA' ? 4 : 2} className={`${inputClass} ${placeholder === 'AAAA' ? 'w-16' : 'w-12'} text-center`} /></span>)}</div></div>;
}

export function isSessionOwnerOrAdmin(session: any, user: any): boolean {
...
export function getMatchStateSnapshot(
...
export async function executeAbandonSession(
  request: NextRequest,
  matchId: string,
  sessionId: string,
  ...
  if (!isSessionOwnerOrAdmin(session, user)) {
  ...
  if (isSessionEnded(session.status)) {
    return NextResponse.json({ message: 'Session already ended', id: session.id, status: session.status });
  }

  const parsed = MarkSessionAbandonedInputSchema.safeParse(await request.json().catch(() => ({})));
  if (!parsed.success) {
    return NextResponse.json({ error: 'INVALID_INPUT', details: parsed.error.errors }, { status: 400 });
  }

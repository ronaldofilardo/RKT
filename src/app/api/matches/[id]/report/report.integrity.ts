...
export function buildReportIntegrity(pointLogs: PointLogRow[], points: TimelinePoint[]): ReportIntegrity {
  ...
  const outOfOrderSequence = pointLogs.filter((p, i) => {
  ...
  if (outOfOrderSequence > 0) warnings.push(`${outOfOrderSequence} ponto(s) com sequência fora de ordem.`);
  ...
  const status = missingSequenceCount > 0 ? 'LEGACY_SEQUENCE' : outOfOrderSequence > 0 ? 'LEGACY_SEQUENCE' : pointsWithoutAnnotationDetails > 0 ? 'INCOMPLETE_ANNOTATION' : 'OK';

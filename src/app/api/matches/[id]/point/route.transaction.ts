...
export async function processPointTransaction(tx: Prisma.TransactionClient, id: string, parsed: PointInput): Promise<TransactionResult> {
  ...
  if (parsed.type === 'DOUBLE_FAULT' && parsed.winnerId === parsed.serverId) {
    throw new TransactionError('Em um double fault, o vencedor do ponto deve ser o receptor, não o sacador', 400, 'DF_WINNER_MUST_BE_RECEIVER');
  }
  ...
  const finishData = newState.isFinished
    ? {
    ...
    : {};
  const updatedMatch = await tx.match.update({ where: { id, version: expectedVersion }, data: { scoreState: snapshot as unknown as Prisma.InputJsonValue, version: { increment: 1 }, ...finishData } });

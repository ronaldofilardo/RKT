    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm border-b safe-top">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
            ...
            <button onClick={onLogout} className="text-sm text-gray-600 hover:text-gray-900 px-2 py-2 sm:px-0 sm:py-0">
              Sair
            </button>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:py-8">
        ...
        <div className="mt-6 sm:mt-8">
          <button onClick={onBack} className="text-sky-600 hover:text-sky-700">
...
function roleClass(role: string) {
  return role === 'ADMIN' ? 'bg-purple-100 text-purple-700' : 'bg-sky-100 text-sky-700';
}
...
function UsersTable({
  ...
  onStartEdit,
  onDelete,
}: Omit<Props, 'error' | 'showUsers' | 'onLogout' | 'onToggleUsers' | 'onBack'>) {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-600">
            <tr>
              {['Nome', 'Email', 'CPF', 'Perfil', 'Clube', 'Ações'].map((label) => (
                <th key={label} className="text-left px-4 sm:px-6 py-3 font-medium">
                  {label}
                </th>
              ))}
            </tr>
...
function UserRow({
  ...
  const editing = editingUser === user.id;
  return (
    <tr className="hover:bg-gray-50">
      <td className="px-4 sm:px-6 py-3 font-medium">{user.name}</td>
      <td className="px-4 sm:px-6 py-3 text-gray-500">{user.email}</td>
      <td className="px-4 sm:px-6 py-3 text-gray-500">{user.cpf || '-'}</td>
      <td className="px-4 sm:px-6 py-3">
        {editing ? (
          ...
          <span className={`px-2 py-0.5 rounded text-xs font-semibold ${roleClass(user.role)}`}>
            {user.role === 'ANNOTATOR' ? 'ANOTADOR' : user.role}
          </span>
        )}
      </td>
      <td className="px-4 sm:px-6 py-3 text-gray-500">{user.club || '-'}</td>
      <td className="px-4 sm:px-6 py-3 text-right whitespace-nowrap">
        {editing ? (
          <>
            <button onClick={() => onSaveRole(user.id)} className="text-xs text-green-600 hover:underline">
              Salvar
            </button>
            <button onClick={onCancelEdit} className="text-xs text-gray-500 hover:underline ml-2">
              Cancelar
            </button>
          </>
        ) : (
          <>
            <button onClick={() => onStartEdit(user)} className="text-xs text-sky-600 hover:underline">
              Alterar Perfil
            </button>
            <button onClick={() => onDelete(user.id)} className="text-xs text-red-600 hover:underline ml-2">
              Excluir
            </button>
          </>
        )}

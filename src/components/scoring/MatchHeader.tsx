  ...
  return (
    <div className="bg-white border-b border-gray-200 px-2 sm:px-4 py-2 sm:py-3">
      <div className="flex items-center justify-between gap-1 sm:gap-2">
        {!isFinished && (
          <button onClick={onClose} className="p-1.5 sm:p-2 text-gray-600 hover:text-gray-900 -ml-1.5 sm:-ml-2 min-h-[36px] min-w-[36px] flex items-center justify-center" aria-label="Fechar">
        ...
        <div className="flex items-center gap-1 sm:gap-2 flex-1 justify-center min-w-0">
          ...
          <span className="text-[10px] sm:text-xs font-semibold text-gray-500 uppercase whitespace-nowrap">{sportType || 'TÊNIS'}</span>
          {elapsedSeconds > 0 && (
            <span className="text-[10px] sm:text-xs font-mono text-gray-500 whitespace-nowrap">⏱ {formatTime(elapsedSeconds)}</span>
          )}
        </div>

        <div className="flex items-center gap-1 flex-shrink-0">
          {canEdit && onEditMatch && (
            <button onClick={onEditMatch} className="p-1.5 sm:p-2 text-gray-500 hover:text-gray-700 min-h-[36px] min-w-[36px] flex items-center justify-center" aria-label="Editar partida">
          ...
          {onStats && (
            <button onClick={onStats} className="p-1.5 sm:p-2 text-gray-500 hover:text-gray-700 text-base sm:text-lg leading-none min-h-[36px] min-w-[36px] flex items-center justify-center" aria-label="Estatísticas">≡</button>
          )}
          {onTimeline && (
            <button onClick={onTimeline} className="px-1.5 sm:px-2 py-1 sm:py-1.5 text-[10px] sm:text-xs font-semibold text-sky-600 hover:text-sky-800 bg-sky-50 hover:bg-sky-100 rounded border border-sky-200 min-h-[32px]">
              📊
            </button>
          )}
        </div>

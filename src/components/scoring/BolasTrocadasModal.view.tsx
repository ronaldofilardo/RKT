type Props = { fontScale: number; bolas: string; handleKeyPress: (value: string) => void; handleBackspace: () => void; handleConfirm: () => void; handleCancel: () => void; };
...
export function BolasTrocadasModalView({ fontScale, bolas, handleKeyPress, handleBackspace, handleConfirm, handleCancel }: Props) {
  return <div className="fixed inset-0 z-[2000] flex items-center justify-center" style={{ backgroundColor: 'rgba(0,0,0,0.5)', backdropFilter: '-webkit-backdrop-filter blur(4px)', WebkitBackdropFilter: 'blur(4px)' }} onClick={handleCancel} role="button" tabIndex={-1} aria-label="Fechar modal" onKeyDown={(e) => { if (e.key === 'Escape' || e.key === 'Enter') handleCancel(); }}>
    <div className="animate-[fadeInSlideUp_0.2s_ease-out] w-[clamp(160px,65vw,210px)] mx-4 rounded-[14px] shadow-[0_20px_60px_rgba(0,0,0,0.5),0_0_30px_rgba(59,130,246,0.15)] flex flex-col" style={{ backgroundColor: 'var(--court-surface)', border: '1px solid rgba(255,255,255,0.1)', borderTop: '3px solid #3b82f6', fontFamily: 'var(--font-main)', fontSize: 'calc(var(--sb-scale) * 0.75em)' }} role="dialog" aria-label="Modal de troca de bolas" onClick={(e) => e.stopPropagation()}>

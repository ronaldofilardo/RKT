      ...
      <table className="w-full">
        <thead>
          <tr className="text-[10px] text-gray-500 dark:text-gray-400 font-mono">
            ...
            {Array.from({ length: numSets }).map((_, i) => {
              ...
              return (
                <th key={i} className={`text-center px-2 py-1 w-8 sm:w-10 ${isCurrent ? 'font-bold text-gray-700 dark:text-gray-300' : ''}`}>
        ...
        <tbody>
          ...
          <tr className="text-xs">
            <td className={`text-left py-2 pr-4 font-semibold ${isSuspended ? 'text-amber-900 dark:text-amber-200' : 'text-gray-900 dark:text-gray-100'}`}>
              {player1.name}
            </td>
            {Array.from({ length: numSets }).map((_, i) => {
              const set = sets[i];
              ...
              return (
                <td
                  ...
                  className={`text-center px-2 py-2 text-sm font-mono ${
                    isCurrent
                      ? isSuspended ? 'text-amber-700 dark:text-amber-300 font-bold' : 'text-gray-900 dark:text-gray-100 font-bold'
                      : isComplete
                        ? isSuspended ? 'text-amber-600 dark:text-amber-400' : 'text-gray-700 dark:text-gray-300'
        ...
        <tfoot>
          ...
          <tr className="text-[9px] border-t border-gray-200 dark:border-gray-700 mt-1">
            <td className="text-left py-2 pr-4 text-gray-400 dark:text-gray-500">Sets</td>
            {Array.from({ length: numSets }).map((_, i) => {
              const set = sets[i];
              const isCurrent = i === currentSetIndex;
              const isComplete = set && (set.player1 > set.player2 || set.player2 > set.player1);

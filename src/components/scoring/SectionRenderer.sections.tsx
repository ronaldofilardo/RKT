...
export function EfeitoSection({ form, vencedor, dispatch, refs, needsSubtipo1 }: { form: PointDetailsForm; vencedor: Vencedor; dispatch: Dispatch; refs: Refs; needsSubtipo1: boolean }) {
...
export function DuracaoSection({ form, dispatch, refs, needsSubtipo1, needsEfeito }: { form: PointDetailsForm; dispatch: Dispatch; refs: Refs; needsSubtipo1: boolean; needsEfeito: boolean }) {
  if (!shouldShowDuracao(form.situacao, form.golpe, form.subtipo1)) return null;
  const num = String(3 + Number(needsSubtipo1) + Number(needsEfeito) + 1);

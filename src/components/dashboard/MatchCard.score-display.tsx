...
function displayScore(set: any, player: 'player1' | 'player2', isMatchTiebreak: boolean, index: number) {
  if (set.isTiebreak && set.tiebreakScore) return set.tiebreakScore[player];
  if (isMatchTiebreak && index === 0) return set[player] ?? 0;
  return set[player] ?? 0;
}

function ScoreCell({ value, suspended }: { value: any; suspended: boolean }) {
...
function SetRows({ sets, player, isMatchTiebreak, suspended }: { sets: any[]; player: 'player1' | 'player2'; isMatchTiebreak: boolean; suspended: boolean }) {
...
function CompletedSetsGrid({ sets, isMatchTiebreak, suspended }: { sets: any[]; isMatchTiebreak: boolean; suspended: boolean }) {
...
function ActiveSetsGrid({ scoreState, isCurrentSetMT, isMatchTiebreak, suspended }: { scoreState: any; isCurrentSetMT: boolean; isMatchTiebreak: boolean; suspended: boolean }) {
...
function CurrentGameGrid({ currentGame, suspended }: { currentGame: any; suspended: boolean }) {
...
export function ScoreDisplay({ scoreState, isFinished, isMatchTiebreak, isCurrentSetMT, suspended }: { scoreState: any; isFinished: boolean; isMatchTiebreak: boolean; isCurrentSetMT: boolean; suspended: boolean }) {
...
export function PlayerNames({ match, hasScore }: { match: any; hasScore: boolean }) {
